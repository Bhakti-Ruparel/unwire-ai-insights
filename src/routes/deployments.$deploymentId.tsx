...
import type { DeploymentRun, DeploymentLog, DeploymentStep } from "@/types/project";
import {
...
function DeploymentDetailPage() {
  ...
  useEffect(() => {
    ...
    apiGetDeployment(deploymentId)
      .then((d) => {
        if (!d) { setError("Deployment not found."); return; }
        setDep(d);
      })
  ...
  // Start SSE stream for live logs when deployment is active
  useEffect(() => {
    if (!dep) return;
    ...
    if (isActive && !streaming) {
      ...
      esRef.current = openDeploymentLogStream(
        ...
        (finalStatus) => {
          setStreaming(false);
          // Refresh deployment record to get final steps state
          apiGetDeployment(deploymentId).then((d) => { if (d) setDep(d); });
        },
        () => {
          setStreaming(false);
    ...
    // For terminal states — load existing logs once via poll
  ...
  async function handleRollback() {
    if (!dep) return;
    if (!confirm("Create a rollback deployment?")) return;
    try {
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <div className="flex items-center gap-3">
                ...
                <span className={`text-sm font-medium ${statusColor} ${isActive ? "animate-pulse" : ""}`}>
                  ● {dep.status}
                </span>
              </div>
                ...
                dep.steps.map((step, i) => (
                  <div key={step.id} className="flex items-start gap-3 py-2">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className={`text-sm font-medium ${step.status === "running" ? "text-blue-400" : step.status === "failed" ? "text-red-400" : step.status === "success" ? "text-foreground" : "text-muted-foreground"}`}>
                        {step.name}
                      </div>
                      <div className="flex items-center gap-2 mt-0.5">
                        {step.durationMs && (
                          <span className="text-xs text-muted-foreground font-mono">{(step.durationMs / 1000).toFixed(1)}s</span>
                        )}
                        {step.error && (
            ...
            <div className="flex-1 overflow-y-auto p-4 font-mono text-xs space-y-0.5 bg-background/50" style={{ maxHeight: "500px" }}>
              ...
              {logs.map((log) => (
                <div key={log.id} className="flex gap-3 leading-5">
                  ...
                  {log.stepName && (
                    <span className="text-accent/70 shrink-0">[{log.stepName}]</span>
                  )}
                  <span className={`break-all ${LOG_COLOR[log.level] ?? "text-foreground/80"}`}>
                    {log.message}
                  </span>
                </div>
              ))}
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <div className="mt-2 text-xs text-muted-foreground">
                  Check the logs above for details. You can ask the AI Chat "Why did deployment v{dep.version} fail?" for an explanation.
                </div>
              </div>
            </div>
          </div>
